    ...
    window.Manager = {
        ...
        delAllSelectedVal:function(e){
            ...
            Tool.layer_confirm("确认删除选中的数据?",function () {
                Ajax.submit("delEntry", {
                    url: "/train_apply/delSelectedBox",
        ...
        setAllStatusReaded:function (e) {
            ...
            if("" == spCodesTemp){
                Tool.layer_alert("请选择需要置为已读的项");
                return false;
            }

            var index = Tool.layer_confirm("确认批量改变阅读状态?",function () {
                Tool.layer_close(index);
                Ajax.submit("updateEntryReadFlag", {
                    url: "/train_apply/updeateReadFlagList",
        ...
        exportExcel:function () {
            ...
            Ajax.query("getData",{
                url:"/train_apply/checkExport?realName="+realName+"&sex="+sex+"&readFlag="+readFlag+"&idNo="+idNo,
                callback:function (result) {
                    if(result=="success"){
                        window.location.href = path +  "/train_apply/export?realName="+realName+"&sex="+sex+"&readFlag="+readFlag+"&idNo="+idNo;
                    }else{
