    ...
    function getChange(v) {
        ...
        with (ss) {
            ...
            $("#"+s[v]+"-fancy").remove();
            $("#"+s[v]).show();
            $("select[name='"+s[v]+"']").fancyspinbox();
            if (++v < s.length) {getChange(v);}
        }

    }
    function geoPreselect(p_key) {
    ...
    var opt0 = ["省份", "地级市", "县区"];
    function geoSetup(id) {
        if(id == null || id == "" || id == "undefined"){
            id = 12;
        }
        for (i = 0; i < s.length - 1; i++) {
            document.getElementById(s[i]).onchange = new Function("getChange(" + (i + 1) + ");");
        }
        getChange(0);
        var myselect = $("select[name='"+s[0]+"']").fancyspinbox();
        myselect.data("fancyspinbox").select(id);
    }
