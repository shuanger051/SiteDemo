...
public class CompetitionSquareProvider extends BaseDynaSqlProvider{

    //alias
    public static final String TABLE_ALIAS = "t_competition_square";
     ...
     * 查询多个结果集
     */
    public String list(final CompetitionSquareDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            if (null != bean){
     ...
     *  保存单个对象
     */
    public String save(final CompetitionSquareDO bean){
        return new SQL() {{
     ...
     * 更新对象
     */
    public String update(final CompetitionSquareDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            if (bean.getTitle() != null) {
                SET("title=#{title}");
     ...
     * 按条件移除对象
     */
    public String remove(final CompetitionSquareDO bean){
        return new SQL(){{
