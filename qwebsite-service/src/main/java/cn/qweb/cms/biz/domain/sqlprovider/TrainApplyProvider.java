...
public class TrainApplyProvider extends BaseDynaSqlProvider{

    public static final String TABLE_ALIAS = "t_train_apply";

    public static final String JOIN_TABLE_ALIAS = "t_train";
     ...
     * 查询多个结果集
     */
    public String list(final TrainApplyDO bean){
        return new SQL(){{
            SELECT(getField(leftFields));
            FROM(TABLE_ALIAS + " a ");
            LEFT_OUTER_JOIN(JOIN_TABLE_ALIAS + " b ON a.content_id = b.id");
            if (null != bean){
                ...
                if(bean.getIdNo()!=null && bean.getIdNo() != ""){
                    WHERE("a.id_no=#{idNo}");
                }
     ...
     *  保存单个对象
     */
    public String save(final TrainApplyDO bean){
        return new SQL() {{
     ...
     * 更新对象
     */
    public String update(final TrainApplyDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            if (bean.getChannelId() != null) {
     ...
     * @return
     */
    public String updateIndexTime(final TrainApplyDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
                SET("gmt_index = null");
            WHERE("id = #{id}");
     ...
     * 按条件移除对象
     */
    public String remove(final TrainApplyDO bean){
        return new SQL(){{
     ...
     * 查询多个结果集
     */
    public String queryUnIndexList(final TrainApplyDO bean){
        return new SQL(){{
            SELECT(getField(leftFields));
            FROM(TABLE_ALIAS + " a ");
            LEFT_OUTER_JOIN(JOIN_TABLE_ALIAS + " b ON a.content_id = b.id");
     ...
     * 查询多个结果集
     */
    public String queryIndexList(final TrainApplyDO bean){
        return new SQL(){{
            SELECT(getField(leftFields));
            FROM(TABLE_ALIAS + " a ");
            LEFT_OUTER_JOIN(JOIN_TABLE_ALIAS + " b ON a.content_id = b.id");
     ...
     * @return
     */
    public String checkExport(final TrainApplyDO bean){
        return new SQL(){{
            SELECT(" count(*) ");
            FROM(TABLE_ALIAS + " a ");
            LEFT_OUTER_JOIN(JOIN_TABLE_ALIAS + " b ON a.content_id = b.id");
