...
public class CooperativePartnerProvider extends BaseDynaSqlProvider{

    //alias
    public static final String TABLE_ALIAS = "t_cooperative_partner";

    public static final String[] Fields={"id","site_name","site_link","friendlinkctg_id","logo","email","description","views","is_enabled","priority","gmt_create","gmt_modified"};
     ...
     * 查询多个结果集
     */
    public String list(final CooperativePartnerDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            if (null != bean){
     ...
     *  保存单个对象
     */
    public String save(final CooperativePartnerDO bean){
        return new SQL() {{
     ...
     * 更新对象
     */
    public String update(final CooperativePartnerDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            if (bean.getSiteName() != null) {
     ...
     * 按条件移除对象
     */
    public String remove(final CooperativePartnerDO bean){
        return new SQL(){{
