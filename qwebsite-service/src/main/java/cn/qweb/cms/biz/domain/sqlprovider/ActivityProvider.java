...
public class ActivityProvider extends BaseDynaSqlProvider{

    //alias
    public static final String TABLE_ALIAS = "t_activity";
     ...
     * 查询多个结果集
     */
    public String list(final ActivityDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            if (null != bean){
     ...
     *  保存单个对象
     */
    public String save(final ActivityDO bean){
        return new SQL() {{
     ...
     * 更新对象
     */
    public String update(final ActivityDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            if (bean.getTitle() != null) {
                SET("title=#{title}");
     ...
     * 按条件移除对象
     */
    public String remove(final ActivityDO bean){
        return new SQL(){{
     ...
     * 查询多个结果集
     */
    public String queryIndexList(final ActivityDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            WHERE("gmt_index is not null");
     ...
     * 查询多个结果集
     */
    public String queryUnIndexList(final ActivityDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            WHERE("gmt_index is null");
     ...
     * @return
     */
    public String updateIndexTime(final ActivityDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            SET("gmt_index = null");
            WHERE("id = #{id}");
