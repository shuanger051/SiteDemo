...
public class ContentAttrProvider extends BaseDynaSqlProvider{

    //alias
    public static final String TABLE_ALIAS = "t_content_attr";

    public static final String[] Fields={"id","content_id","attr_name","attr_value","gmt_create","gmt_modified"};
     ...
     * 查询多个结果集
     */
    public String list(final ContentAttrDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            if (null != bean){
     ...
     *  保存单个对象
     */
    public String save(final ContentAttrDO bean){
        return new SQL() {{
     ...
     * 更新对象
     */
    public String update(final ContentAttrDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            if (bean.getContentId() != null) {
     ...
     * 按条件移除对象
     */
    public String remove(final ContentAttrDO bean){
        return new SQL(){{
