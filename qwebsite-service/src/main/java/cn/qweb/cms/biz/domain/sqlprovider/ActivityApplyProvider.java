...
public class ActivityApplyProvider extends BaseDynaSqlProvider{

    //alias
    public static final String TABLE_ALIAS = "t_activity_apply";
    public static final String JOIN_TABLE_ALIAS = "t_activity";
     ...
     * 查询多个结果集
     */
    public String list(final ActivityApplyDO bean){
        return new SQL(){{
            SELECT(getField(leftFields));
            FROM(TABLE_ALIAS + " a ");
            LEFT_OUTER_JOIN(JOIN_TABLE_ALIAS + " b ON a.content_id = b.id");
     ...
     *  保存单个对象
     */
    public String save(final ActivityApplyDO bean){
        return new SQL() {{
     ...
     * 更新对象
     */
    public String update(final ActivityApplyDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            if (bean.getChannelId() != null) {
     ...
     * 按条件移除对象
     */
    public String remove(final ActivityApplyDO bean){
        return new SQL(){{
     ...
     * @return
     */
    public String checkExport(final ActivityApplyDO bean){
        return new SQL(){{
            SELECT(" count(*) ");
            FROM(TABLE_ALIAS + " a ");
            LEFT_OUTER_JOIN(JOIN_TABLE_ALIAS + " b ON a.content_id = b.id");
