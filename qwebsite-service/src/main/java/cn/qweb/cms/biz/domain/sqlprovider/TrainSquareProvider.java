...
public class TrainSquareProvider extends BaseDynaSqlProvider{

    //alias
    public static final String TABLE_ALIAS = "t_train_square";
     ...
     * 查询多个结果集
     */
    public String list(final TrainSquareDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            if (null != bean){
     ...
     *  保存单个对象
     */
    public String save(final TrainSquareDO bean){
        return new SQL() {{
     ...
     * 更新对象
     */
    public String update(final TrainSquareDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            if (bean.getTitle() != null) {
                SET("title=#{title}");
     ...
     * 按条件移除对象
     */
    public String remove(final TrainSquareDO bean){
        return new SQL(){{
     ...
     * 查询多个结果集
     */
    public String queryUnIndexList(final TrainSquareDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            WHERE("gmt_index is null");
     ...
     * 查询多个结果集
     */
    public String queryIndexList(final TrainSquareDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            WHERE("gmt_index is not null");
     ...
     * @return
     */
    public String updateIndexTime(final TrainSquareDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            SET("gmt_index = null");
            WHERE("id = #{id}");
