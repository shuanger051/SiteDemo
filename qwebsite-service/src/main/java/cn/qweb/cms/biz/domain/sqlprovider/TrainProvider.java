...
public class TrainProvider extends BaseDynaSqlProvider{

    //alias
    public static final String TABLE_ALIAS = "t_train";
     ...
     * 查询多个结果集
     */
    public String list(final TrainDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            if (null != bean){
     ...
     *  保存单个对象
     */
    public String save(final TrainDO bean){
        return new SQL() {{
     ...
     * 更新对象
     */
    public String update(final TrainDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            if (bean.getTitle() != null) {
                SET("title=#{title}");
     ...
     * 按条件移除对象
     */
    public String remove(final TrainDO bean){
        return new SQL(){{
     ...
     * 查询多个结果集
     */
    public String queryUnIndexList(final TrainDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            WHERE("gmt_index is null");
     ...
     * 查询多个结果集
     */
    public String queryIndexList(final TrainDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            WHERE("gmt_index is not null");
     ...
     * @return
     */
    public String updateIndexTime(final TrainDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            SET("gmt_index = null");
            WHERE("id = #{id}");
