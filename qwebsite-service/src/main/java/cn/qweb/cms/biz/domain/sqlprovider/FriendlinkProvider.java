...
public class FriendlinkProvider extends BaseDynaSqlProvider{

    //alias
    public static final String TABLE_ALIAS = "t_friendlink";
     ...
     * 查询多个结果集
     */
    public String list(final FriendlinkDO bean){
        return new SQL(){{
            SELECT(getField(Fields));
            FROM(TABLE_ALIAS);
            if (null != bean){
     ...
     *  保存单个对象
     */
    public String save(final FriendlinkDO bean){
        return new SQL() {{
     ...
     * 更新对象
     */
    public String update(final FriendlinkDO bean){
        return new SQL(){{
            UPDATE(TABLE_ALIAS);
            if (bean.getSiteName() != null) {
     ...
     * 按条件移除对象
     */
    public String remove(final FriendlinkDO bean){
        return new SQL(){{
