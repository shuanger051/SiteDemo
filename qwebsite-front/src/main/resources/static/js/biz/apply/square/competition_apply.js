...
var TrainApplyModel = function ($){
    ...
    var Service = {
        _get:function (id){
            Ajax.query("getTrain",{
                url:"/square_apply/competition/get",
                data:{id:id},
        ...
        _queryDictItem:function (){
            Ajax.query("getItem",{
                ...
                data:{
                    "entryCodes":"projectSquareKind"
                },
                callback:function (result){
                    $("#projectKindSpan").html(template("selectTemp",{list:result.projectSquareKind}));
                }
            })
        },
        _queryTeamTypeDictItem:function () {
            Ajax.query("getItem1",{
                url:"/dict/list_by_entry_code",
                data:{
                    "entryCodes":"teamSquareType"
                },
                callback:function (result){
                    $("#teamTypeSelect").html("");
                    $("#teamTypeSelect").html(result.teamSquareType[0].itemName);
                    $("#teamType").val(result.teamSquareType[0].itemCode);
                }
            })
        }
    }
