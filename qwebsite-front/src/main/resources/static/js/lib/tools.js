 ...
 */
    var Tool = {
                ...
                if(nodes.size() > 0){
                    ...
                    if("input" == tagName){
                        ...
                        // for(var i = 0;i<selectLength;i++){
                            $form.find("select[name='" + name + "'] option[value='"+value+"']")[0].selected = true;
                        // }
                    } else if("img" == tagName){
        ...
        initFileUpload:function (opts) {
            ...
            opts.url = opts.url + "?isAjax=true&_="+Math.random();
        ...
        tipType:function (msg,o,cssctl) {
            if (o.type == 3) {//失败
                $("#alert").find("#alert_error_content").find("h5").html(msg).end().show().end().addClass('is-visible2');
            } else {
                $("#alert").find(".png_img").hide().end().removeClass('is-visible2');
            }
            if(!!o.callback){
                $("#closeAlert").unbind().bind("click",function (){
                    Tool.closeAlert();
                    o.callback();
                });
            }else{
        ...
        tipSuccess:function (msg,o){
            $("#alert").find("#alert_ok_content").find("h5").html(msg).end().show().end().addClass('is-visible2');
            if(!!o.callback){
                $("#closeAlert").unbind().bind("click",function (){
                    Tool.closeAlert();
                    o.callback();
